...
def _exit_spec_mode_input_key(tool_input: dict[object, object]) -> str:
    serialized = json.dumps(tool_input, sort_keys=True, default=str, separators=(",", ":"))
    return f"input:{sha256(serialized.encode('utf-8')).hexdigest()}"


def _exit_spec_mode_dedupe_keys(proposal_id: object, tool_input: dict[object, object]) -> list[str]:
    keys: list[str] = []
    if isinstance(proposal_id, str) and proposal_id.strip():
        keys.append(f"id:{proposal_id.strip()}")
    keys.append(_exit_spec_mode_input_key(tool_input))
    return keys


def _iter_exit_spec_mode_blocks(entry: dict) -> list[tuple[list[str], dict]]:
    ...
    for block in content:
        ...
        if not isinstance(tool_input, dict):
            continue
        blocks.append((_exit_spec_mode_dedupe_keys(block.get("id"), tool_input), tool_input))
    return blocks


def extract_exit_spec_mode_notification_events_from_line(line: str) -> list[tuple[list[str], str]]:
    try:
    ...
    for dedupe_keys, tool_input in _iter_exit_spec_mode_blocks(entry):
        summary = _summarize_exit_spec_mode(tool_input)
        if summary:
            notifications.append((dedupe_keys, summary))
    return notifications


def extract_exit_spec_mode_notifications_from_line(line: str) -> list[tuple[str, str]]:
    notifications: list[tuple[str, str]] = []
    for dedupe_keys, summary in extract_exit_spec_mode_notification_events_from_line(line):
        proposal_id = next((key.removeprefix("id:") for key in dedupe_keys if key.startswith("id:")), dedupe_keys[0])
        notifications.append((proposal_id, summary))
    return notifications


def extract_exit_spec_mode_dedupe_keys(input_data: dict, droid_event: str) -> list[str]:
    if droid_event == "PreToolUse" and input_data.get("tool_name") == "ExitSpecMode":
        tool_input = input_data.get("tool_input")
        if isinstance(tool_input, dict):
            return _exit_spec_mode_dedupe_keys(input_data.get("tool_use_id") or input_data.get("toolUseId"), tool_input)

    if droid_event != "Notification":
        return []

    keys: list[str] = []
    for source in (input_data, input_data.get("metadata"), input_data.get("request")):
        if not isinstance(source, dict):
            continue
        for block_keys, _ in _iter_exit_spec_mode_blocks(source):
            keys.extend(block_keys)
    return list(dict.fromkeys(keys))


def mark_exit_spec_mode_seen(session_key: str, dedupe_keys: list[str]) -> None:
    if not dedupe_keys:
        return
    state = _read_spec_watcher_state(session_key)
    seen_ids = {str(value) for value in state.get("seen_ids", []) if isinstance(value, str)}
    seen_ids.update(dedupe_keys)
...
def has_seen_exit_spec_mode(session_key: str, dedupe_keys: list[str]) -> bool:
    if not dedupe_keys:
        return False
    state = _read_spec_watcher_state(session_key)
    seen_ids = {str(value) for value in state.get("seen_ids", []) if isinstance(value, str)}
    return any(key in seen_ids for key in dedupe_keys)
